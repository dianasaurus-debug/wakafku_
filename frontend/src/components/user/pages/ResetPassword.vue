...
export default {
  ...
  methods: {
    submitPassword(){
      ...
      this.$Progress.start()
      axios({method : 'post', url : URL + '/api/wakif-auth/resetPassword', data: { token : this.password_reset.code, password : this.password_reset.password, email : this.requestEmail}})
          .then((res) => {
                console.log(res)
                this.state=1;
    ...
    submitEmail() {
      const formData = new FormData()
      this.$Progress.start()
      axios({method : 'post', url : URL + '/api/wakif-auth/requestResetPassword/', data: { email : this.requestEmail}})
          .then((res) => {
                console.log(res)
                this.state=1;
                this.$emit('AfterCreated')
